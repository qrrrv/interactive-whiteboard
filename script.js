...
function initializeCanvas() {
  ...
  canvas.on('object:modified', function(e) {
    if (!isReceivingData) {
      // При изменении объекта (перемещение, ресайз)
      // Лучше всего удалить старый и добавить новый на принимающей стороне, 
      // или передать полные параметры. Для простоты передадим весь холст.
      // Но для оптимизации передадим только измененный объект + его ID (если есть), 
      // но Fabric.js по умолчанию не имеет ID.
      // Для простого демо: отправляем весь стейт (не очень эффективно, но надежно)
      broadcastData({
        type: 'state',
